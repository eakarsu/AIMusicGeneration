...
// (separate vocals/drums/bass)

export default function GapAiAudioStemSeparator() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-ai-audio-stem-separator', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>Ai Audio Stem Separator</h1>
      <p style={{ color: '#555', marginBottom: 20 }}>(separate vocals/drums/bass)</p>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          >
            {loading ? 'Running...' : 'Run Ai Audio Stem Separator'}
          </button>
          <button
