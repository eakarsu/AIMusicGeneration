...
// Route: /api/gap-ai-audio-stem-separator
// Feature: Ai Audio Stem Separator
// Description: (separate vocals/drums/bass)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Audio Stem Separator. (separate vocals/drums/bass)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Audio Stem Separator\nDescription: (separate vocals/drums/bass)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-audio-stem-separator', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-audio-stem-separator', title: 'Ai Audio Stem Separator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-audio-stem-separator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-audio-stem-separator']);
      return res.json({ slug: 'ai-audio-stem-separator', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-audio-stem-separator', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-audio-stem-separator', items: [], error: (e && e.message) || 'Server error' });
  }
});
