// Auto-generated feature route: music-composer-agent
// Domain: agentic music composer — Capability: producing arrangements from natural-language briefs
// Mounted under: /api/music-composer-agent
...
// POST /api/music-composer-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic music composer. Your task is producing arrangements from natural-language briefs. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/music-composer-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/music-composer-agent/:id — fetch one
...
// DELETE /api/music-composer-agent/:id — discard a run
