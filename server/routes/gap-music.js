...
// Route: /api/gap-music
// Feature: Music
// Description: Music theory lesson surface
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Music. Music theory lesson surface' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'music', title: 'Music', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'music', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['music']);
      return res.json({ slug: 'music', items: r.rows || [] });
    }
    return res.json({ slug: 'music', items: [] });
  } catch (e) {
    return res.json({ slug: 'music', items: [], error: (e && e.message) || 'Server error' });
  }
});
