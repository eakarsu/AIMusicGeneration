...
// Route: /api/gap-ai-mastering-engineer
// Feature: Ai Mastering Engineer
// Description: (auto-mastering)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Mastering Engineer. (auto-mastering)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Mastering Engineer\nDescription: (auto-mastering)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-mastering-engineer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-mastering-engineer', title: 'Ai Mastering Engineer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-mastering-engineer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-mastering-engineer']);
      return res.json({ slug: 'ai-mastering-engineer', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-mastering-engineer', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-mastering-engineer', items: [], error: (e && e.message) || 'Server error' });
  }
});
