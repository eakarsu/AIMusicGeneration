// Auto-generated feature route: daw-copilot
// Domain: DAW production co-pilot — Capability: providing real-time production guidance inside a session
// Mounted under: /api/daw-copilot
...
// POST /api/daw-copilot/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a DAW production co-pilot. Your task is providing real-time production guidance inside a session. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/daw-copilot/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/daw-copilot/:id — fetch one
...
// DELETE /api/daw-copilot/:id — discard a run
