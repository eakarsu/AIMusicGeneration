...
// Route: /api/gap-ai-vocal-melody-cloner
// Feature: Ai Vocal Melody Cloner
// Description: (style transfer on vocals)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Vocal Melody Cloner. (style transfer on vocals)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Vocal Melody Cloner\nDescription: (style transfer on vocals)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-vocal-melody-cloner', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-vocal-melody-cloner', title: 'Ai Vocal Melody Cloner', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-vocal-melody-cloner', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-vocal-melody-cloner']);
      return res.json({ slug: 'ai-vocal-melody-cloner', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-vocal-melody-cloner', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-vocal-melody-cloner', items: [], error: (e && e.message) || 'Server error' });
  }
});
