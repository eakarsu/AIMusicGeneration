...
// Route: /api/gap-collaboration
// Feature: Collaboration
// Description: Collaboration / co-writing tools
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Collaboration. Collaboration / co-writing tools' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'collaboration', title: 'Collaboration', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'collaboration', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['collaboration']);
      return res.json({ slug: 'collaboration', items: r.rows || [] });
    }
    return res.json({ slug: 'collaboration', items: [] });
  } catch (e) {
    return res.json({ slug: 'collaboration', items: [], error: (e && e.message) || 'Server error' });
  }
});
