...
if [[ ! -f "$PROJECT_DIR/.env" ]]; then
  echo ".env is required. Copy .env.example and provide the runtime values." >&2
  exit 1
fi
...
if [[ ! -d "$PROJECT_DIR/server/node_modules" || ! -d "$PROJECT_DIR/client/node_modules" ]]; then
  echo "Dependencies absent; run ./scripts/bootstrap.sh." >&2
  exit 1
fi
...
server_pid=$!
(cd "$PROJECT_DIR/client" && BROWSER=none PORT="$CLIENT_PORT" REACT_APP_API_URL="http://127.0.0.1:${SERVER_PORT}" npm start) &
client_pid=$!
cleanup() {
